              ...
              filteredStaff.map(member => {
                ...
                return (
                  <tr key={member.id} className="hover:bg-[#F0EAE3]/20 transition-colors">
                    ...
                    <td className="px-6 py-4">
                      {member.specialization && Array.isArray(member.specialization) && member.specialization.length > 0 ? (
                        <div className="flex flex-wrap gap-1">
                          {member.specialization.map((spec, idx) => (
                            <span key={idx} className="inline-flex items-center px-2 py-0.5 rounded text-[10px] font-medium bg-[#EBE6E0]/50 text-[#524A44] border border-[#EBE6E0]">
                              {spec}
                            </span>
                          ))}
                        </div>
                      ) : member.specialization && typeof member.specialization === 'string' ? (
                        <span className="text-[#524A44] text-sm">{member.specialization}</span>
                      ) : (
                        <span className="text-[#A8A19A] text-xs italic">Not assigned</span>
                      )}
                    </td>
                    <td className="px-6 py-4">
                      {(() => {
